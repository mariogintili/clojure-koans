...
(defmacro infix-better [form]
  `(~(second form) ; Note the syntax-quote (`) and unquote (~) characters!
    ~(first form)
; Something cool that I learned today:
; subexpressions are not expanded by default
; meaning that when you call: (infix-better ( 10 + (2 * 3))))
; the (nth form 2) will be (2 * 3) and not 6!
; resulting in (+ 10 (2 * 3))
    ~(nth form 2)))
